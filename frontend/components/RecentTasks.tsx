...
export default function RecentTasks() {
  ...
  if (error) {
    return (
      <Card className="w-full">
        <CardHeader className="flex flex-col items-center py-6">
          <Activity className="h-5 w-5 text-destructive mr-2}">Error loading tasks</CardTitle>
        </CardHeader>
      </Card>
    );
  }
    ...
    <Card className="w-full">
      <CardHeader className="pb-4">
        ...
        <p className="text-xs text-muted-foreground">
          Latest system and workflow activities
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        {tasks.map((task) => (
          <div key={task.id || Math.random()} className="bg-card/50 p-4 rounded-lg border border-border/50 flex items-center space-x-4">
            <div className="w-10 h-10 bg-brass-500/10 rounded-flex items-center justify-center">
              ...
              {!['workflow', 'approval', 'notification', 'completed'].includes(task.type || '') && (
